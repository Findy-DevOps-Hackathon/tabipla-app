...
import { searchDestinationPlaces } from "../data/places.ts";
...
export function InputScreen({ afterDiagnosis = false, onBack, onSearch }: InputScreenProps) {
  ...
  const placeMatches = location.length > 0 ? searchDestinationPlaces(location) : [];
  const hasDestinationMatch = placeMatches.some((place) => place.name === location);
  const canSearch = hasDestinationMatch && !locating;
