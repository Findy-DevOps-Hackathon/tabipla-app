...
AI 機能はすべて backend-api 経由です（agent URL はフロントに露出しません）。
...
開発時は Vite dev server が `/api/*` を backend-api へプロキシします。
...
- admin-web は Firebase プロジェクトが `tabipla-admin-web` のため、`/api` rewrite は使えません。
  代わりに `VITE_API_BASE` で Cloud Run backend-api を直接指します（AI 機能も同経路）。
...
1. **個別登録** — フォーム入力 + Places lookup（住所の自動補完）
