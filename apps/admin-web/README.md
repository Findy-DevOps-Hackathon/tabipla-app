...
- **`/api/*`（backend-api）と `/agent/*`（収集エージェント）は本番では未接続**。
  これらのプロキシは開発サーバ（`vite.config.ts`）限定のため、Firebase Hosting 単体では
  ログイン・CRUD・AI 収集は動きません。画面遷移・UI の確認のみ可能です。
- **本番で API を繋ぐ場合**は、`backend-api` と `agent` を Cloud Run 等へデプロイし、
  `firebase.json` の `rewrites` に `/api/**`・`/agent/**` への転送を追加する。
  または `src/api.ts` の `BASE` / `CollectPage` の `AGENT_URL` を本番 URL に切り替える。
