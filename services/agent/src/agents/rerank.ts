...
const attendPlanOutputSchema = z.object({
  ...
  result: z.string().describe("今回の旅程の魅力、ストーリーラインを語るワクワクする日本語アテンド紹介文（70〜100文字程度、最大100文字厳守）。「どのような要素やユーザーの好みを重視して今回のスポットを選んだのか」「どんなテーマの旅行を提案しているのか」を親しみやすいカジュアルかつ「です・ます」調（丁寧語）で明確に言語化してください。「お客様」などの固い敬語は使用禁止です。単なるスポット名の羅列は厳禁です。"),
...
- 'result' には、今回のプランがユーザーのどのような好み（例: 歴史散策、景色の美しさ、ご当地グルメなど）や旅の要望を重視して構築されたのか、全体のストーリーやテーマ性などを、親しみのあるカジュアルかつ「です・ます」調（丁寧語）で簡潔にまとめた紹介文（70〜100文字程度、最大100文字厳守）を作成してください。「お客様」などの固すぎる敬語表現は避け、フランクでありつつも丁寧で温かみのある案内文にしてください。単にスポット名をカンマ区切りや箇条書きで羅列するだけの文章は絶対に避けてください。
