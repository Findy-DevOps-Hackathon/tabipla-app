...
export function parseDescribeResult(text: string): DescribeResult {
  ...
  const result = describeResultSchema.safeParse(parsed);
  if (!result.success) {
...
export async function describeSpot(
  ...
  const prompt = `次の観光地について紹介文を作成してください。

【観光地名】${input.name}
【都道府県】${input.prefecture}
【市区町村】${input.municipality}${addressHint}

上記市区町村内のスポットとして、検索で所在地を確認してから紹介文を書いてください。`;
