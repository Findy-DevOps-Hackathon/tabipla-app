...
# Cloud SQL Auth Proxy 経由で Drizzle マイグレーションを適用する。
...
if ! command -v "$PROXY_BIN" >/dev/null 2>&1; then
  ...
  echo "  または https://cloud.google.com/sql/docs/postgres/sql-proxy" >&2
  exit 1
fi
...
echo "Running migrations (DATABASE_URL -> 127.0.0.1:${PROXY_PORT})..."
pnpm -C "$ROOT/packages/db" db:migrate

echo ""
echo "Migration complete."
echo "Optional: pnpm -C packages/db seed && pnpm -C services/backend-api reindex"
